 ...
 */
@Component
public class Compare_mw_s1<T extends DVBChannel> extends Service<List<T>> {
    ...
    private void integratePPR(List<T> services, List<T> servicesOld) throws Exception {
        ...
        for (T serviceOld : servicesOld) {
            ...
            if (!isFind) {
            	T service = (T) new DVBChannel(serviceOld.getType(), serviceOld.getIdx(), serviceOld.getName(), serviceOld.getNid(), serviceOld.getPpr(), serviceOld.getLine(), serviceOld.getFlag(), serviceOld.getNeew());
